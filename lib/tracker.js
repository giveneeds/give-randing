...
const SKIP_KEY = 'gvnds.skip_tracking'; // 값이 있으면 trackEvent/startSession 스킵
const SESSION_TIMEOUT = 30 * 60 * 1000; // 30분

// ── 트래킹 스킵 (admin/superadmin 자기 추적 방지) ──
// 탭 단위(sessionStorage)로 저장하여 로그아웃 후 새 탭에서는 다시 정상 트래킹.
export function setSkipTracking(reason) {
  if (typeof window === 'undefined') return;
  try { sessionStorage.setItem(SKIP_KEY, reason || '1'); } catch {}
}

export function clearSkipTracking() {
  if (typeof window === 'undefined') return;
...
export function shouldSkipTracking() {
  ...
  if (isCurrentAdminOrPreviewPath()) return true;
  try { return !!sessionStorage.getItem(SKIP_KEY); } catch { return false; }
}

// ── Anonymous ID ──
export function getAnonymousId() {
  if (typeof window === 'undefined') return null;
  if (shouldSkipTracking()) return null;
...
// ── 세션 시작 (서버에 POST) ──
export async function startSession(extraMeta = {}) {
  if (shouldSkipTracking()) return null;
