...
const MAX_REAL_BODY_SAMPLE_CHARS = 500;
...
function loadThreadsReferenceDetailItems() {
  ...
  try {
    ...
    const files = fs.readdirSync(dir)
      // detail 파일만. -raw- 등 비-detail 은 정규식에서 자연히 제외된다.
      .filter((f) => /^threads-reference-detail-\d{4}-\d{2}-\d{2}-.+\.json$/.test(f))
      .sort();
    const seen = new Set();
    for (const f of files) {
      ...
      for (const it of parsed) {
        ...
        if (rootText.length < REAL_BODY_MIN_LENGTH) continue;
        if (seen.has(rootText)) continue;
        seen.add(rootText);
        const continuations = Array.isArray(it.same_author_continuations)
          ? it.same_author_continuations.length
          : 0;
        items.push({ rootText, author: it.root_author || '', continuations });
      }
    }
  } catch {
    items = [];
...
export function getThreadsRealBodySamples({ queryText, maxSamples = 3, preferSingle = true } = {}) {
  const items = loadThreadsReferenceDetailItems();
  if (items.length === 0) return '';
  const queryTokens = tokenizeReferenceQuery(queryText || '');
  const scored = items.map((it) => {
    const itemTokens = tokenizeReferenceQuery(it.rootText);
    let score = 0;
    for (const token of queryTokens) {
      if (itemTokens.has(token)) score += 1;
    }
    // 단일 발행글(연속 스레드 0개)에 약간의 가산점 — 톤 샘플은 한 호흡 글이 깔끔하다.
    if (preferSingle && it.continuations === 0) score += 0.5;
    return { it, score };
  });
  scored.sort((a, b) => b.score - a.score);
  const top = scored.slice(0, Math.max(1, maxSamples));
  if (top.length === 0) return '';
  return top
    .map((entry, i) => `예시 ${i + 1} (실제 발행글):\n${entry.it.rootText.slice(0, MAX_REAL_BODY_SAMPLE_CHARS)}\n`)
    .join('\n');
}
...
export function buildKnowledgeContext({
  ...
  if (channel === 'threads') {
    ...
    if (includeAudit) {
      const realSamples = getThreadsRealBodySamples({ queryText: referenceQueryText, maxSamples: 3 });
      if (realSamples) blocks.push(`[실제 발행된 Threads 글 — 이 말투·어미·문장 길이·후킹 방식을 그대로 흉내내라. 주제는 새로 쓰되 톤은 이 샘플과 같게]\n${realSamples}`);
