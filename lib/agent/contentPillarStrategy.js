...
// 목적 축(engagement_intent) — 기둥(주제 역할)과 직교. 매일 cron 이 같은 정보형만 내지 않게 다양성 확보용.
const FALLBACK_INTENTS = {
  reach: { label: '유입형', role: '검색·호기심으로 새 사람을 끌어온다', signalKeywords: ['정리', '방법', '가지', '추천', '왜'] },
  trust: { label: '신뢰형', role: '사례·실패담으로 신뢰를 쌓는다', signalKeywords: ['사례', '실패', '후기', '결과', '해봤'] },
  convert: { label: '전환형', role: '자료·진단·문의로 다음 행동을 잇는다', signalKeywords: ['자료', '템플릿', '진단', '상담', '신청'] },
  relate: { label: '관계형', role: '질문·공감으로 댓글을 유발한다', signalKeywords: ['질문', '여러분', '어떻게', '다들', '투표'] },
  recycle: { label: '재활용형', role: '후속편·시리즈로 다시 굴린다', signalKeywords: ['다음 편', '시리즈', '지난번', '이어서'] },
};

let pillarConfigCache = null;
let intentConfigCache = null;

function readPillarConfigFile() {
  try {
...
export function getContentPillars() {
  ...
  const parsed = readPillarConfigFile();
...
export function getEngagementIntents() {
  if (intentConfigCache) return intentConfigCache;
  const parsed = readPillarConfigFile();
  intentConfigCache = parsed?.engagementIntents && typeof parsed.engagementIntents === 'object'
    ? parsed.engagementIntents
    : FALLBACK_INTENTS;
  return intentConfigCache;
}

// 텍스트 신호 + 소스로 목적 축 1개 선택. 신호가 약하면 reach 기본.
export function chooseEngagementIntent({ text = '', source = '' } = {}) {
  const intents = getEngagementIntents();
  const lower = `${text}`.toLowerCase();
  const s = `${source}`.toLowerCase();
  const scored = Object.entries(intents).map(([key, value]) => {
    let score = 0;
    for (const kw of value.signalKeywords || []) {
      if (kw && lower.includes(String(kw).toLowerCase())) score += 1;
    }
    // 소스 보정: reddit/x/news 는 유입형(새 흐름) 쪽으로 약하게.
    if (key === 'reach' && /reddit|x_search|news|article|rss/.test(s)) score += 0.5;
    if (key === 'trust' && /실패|사례|후기|결과/.test(lower)) score += 0.5;
    return { key, label: value.label || key, role: value.role || '', score };
  }).sort((a, b) => b.score - a.score);
  const top = scored[0];
  if (!top || top.score === 0) {
    const reach = intents.reach ? { key: 'reach', label: intents.reach.label, role: intents.reach.role } : { key: 'reach', label: '유입형', role: '' };
    return { ...reach, intent_fit: 'default' };
  }
  return { key: top.key, label: top.label, role: top.role, intent_fit: 'signal' };
}
...
export function choosePillarStrategy({ item, theme, candidate } = {}) {
  ...
  const intent = chooseEngagementIntent({ text, source });
  ...
  return {
    ...
    selected_engagement_intent: intent.key,
    selected_engagement_intent_label: intent.label,
    engagement_intent_role: intent.role,
    ...
    thread_reference_strategy: buildThreadReferenceStrategy({ selected, classification, source, text }),
...
function scoreCandidateForTopicSelection(candidate, selected) {
  ...
  score += threadabilityScore(candidate);
...
function threadabilityScore(candidate) {
  const text = [
    candidate.title,
    candidate.recommended_title,
    candidate.one_line,
  ...
  let score = 0;
  if (/왜|실수|놓치|바뀌|차이|모르는|이미|반응|후회|비용|돈|시간|비교|사례|정리|가지|추천/.test(text)) score += 0.55;
  if (/활용법|전략|효율|성장|기회|중요성/.test(text)) score -= 0.25;
  if (/ai 마케팅|ai 활용|마케팅 전략/.test(text) && !/사례|실수|비용|비교|툴|화면|전환|검색|리뷰/.test(text)) score -= 0.45;
  return score;
}

function buildThreadReferenceStrategy({ selected, classification, source, text }) {
  const mechanism = pickOpeningPattern({ text, source });
  const firstPostHint = buildFirstPostHint({ mechanism, classification, text });
  return {
    opening_pattern: mechanism,
    first_post_rule: '연결글이면 첫 포스트는 80~220자 안에서 궁금증/FOMO/오해 깨기를 만든다. 정보 설명은 2번 포스트부터 둔다.',
    first_post_hint: firstPostHint,
    continuation_plan: buildContinuationPlan({ selected, classification, text }),
    why_threadable: buildThreadableReason({ mechanism, selected, classification }),
  };
}

function pickOpeningPattern({ text, source }) {
  const s = String(source || '').toLowerCase();
  if (/실수|잘못|놓치|못하고|안 되는|안되는/.test(text)) return 'mistake_gap';
  if (/이미|요즘|바뀌|전환|룰|검색|플랫폼|ai/.test(text) || ['reddit_search', 'reddit', 'x_search'].includes(s)) return 'rule_changed';
  if (/돈|비용|광고비|낭비|효율|성과/.test(text)) return 'cost_leak';
  if (/추천|정리|가지|모음|도구|툴|템플릿/.test(text)) return 'resource_promise';
  if (/논란|반응|불만|후기|댓글/.test(text)) return 'curiosity_gap';
  return 'curiosity_gap';
}

function buildFirstPostHint({ mechanism, classification, text }) {
  const angle = classification.content_angle || (Array.isArray(classification.content_angles) ? classification.content_angles[0] : '');
  if (mechanism === 'rule_changed') return `첫 글은 "이제 ${shortenAngle(angle || '마케팅 방식')}이 예전처럼 안 먹히는 이유"처럼 룰이 바뀐 느낌을 먼저 준다.`;
  if (mechanism === 'cost_leak') return '첫 글은 광고비/시간이 새는 장면을 짧게 보여주고, 왜 새는지는 후속 포스트에서 푼다.';
  if (mechanism === 'mistake_gap') return '첫 글은 많이 하는 실수 하나를 짚되, 바로 해결책을 다 말하지 말고 다음 글을 보게 만든다.';
  if (mechanism === 'resource_promise') return '첫 글은 자료/도구/예시를 얻을 이유를 먼저 주고, 목록은 후속 포스트로 넘긴다.';
  return '첫 글은 결론을 다 말하지 말고 독자가 "그래서 왜?"라고 느끼는 빈칸을 만든다.';
}

function buildContinuationPlan({ selected, classification, text }) {
  if (selected.content_pillar === 'do_today') return ['2번: 왜 지금 필요한지', '3번: 따라 할 기준', '4번: 예시/입력값', '5번: 오늘 바꿀 것'];
  if (selected.content_pillar === 'cost_before_spend') return ['2번: 새는 지점', '3번: 확인할 화면/지표', '4번: 흔한 착각', '5번: 돈 쓰기 전 질문'];
  if (selected.content_pillar === 'content_showcase') return ['2번: 안 먹히는 표현', '3번: 바꿔야 할 소재', '4번: 좋은 예시 구조', '5번: 저장할 기준'];
  if (/뉴스|논란|반응|업데이트|출시/.test(text)) return ['2번: 무슨 일이 있었는지', '3번: 사람들이 반응한 이유', '4번: 브랜드/운영자가 볼 지점', '5번: 과장 없이 남길 관점'];
  return ['2번: 배경을 쉽게 설명', '3번: 사람들이 놓치는 지점', '4번: 실제 적용 장면', '5번: 기억할 기준'];
}

function buildThreadableReason({ mechanism, selected, classification }) {
  const label = selected.label || selected.content_pillar;
  if (mechanism === 'resource_promise') return `${label} 안에서 저장형으로 풀 수 있어 후속 포스트를 볼 이유가 생깁니다.`;
  if (mechanism === 'rule_changed') return `${label} 안에서 "예전 방식과 지금 방식의 차이"를 만들 수 있어 첫 글 호기심이 생깁니다.`;
  if (mechanism === 'cost_leak') return `${label} 안에서 돈/시간이 새는 장면을 보여줄 수 있어 FOMO를 과격하지 않게 만들 수 있습니다.`;
  if (mechanism === 'mistake_gap') return `${label} 안에서 흔한 착각을 먼저 보여주고 후속 글에서 기준을 줄 수 있습니다.`;
  return `${label} 안에서 결론보다 궁금증을 먼저 세울 수 있습니다.`;
}

function shortenAngle(value) {
  return String(value || '').replace(/[.!?。]+$/g, '').slice(0, 28);
}
