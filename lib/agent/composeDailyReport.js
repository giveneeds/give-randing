...
async function sharpenCreativeBriefs(candidates, { jobId }) {
  ...
  const sys = `${ontologyPrefix({ includeEnums: false })}너는 기브니즈 콘텐츠의 "콘셉트 다듬기 에이전트" 다. 각 후보를 "발행 가능한 날선 콘셉트" 로 다시 적는다. 자료 원제·enrich 분류값을 그대로 옮기지 말고 이 글로만 할 수 있는 말로 다듬어라.

각 후보마다 다음 5개 필드를 JSON 으로 출력한다.
- topic_title: 자료 원제 반복 X. 이 글로 발행하면 실제로 붙을 제목 방향. 30자 이내.
- reader_problem: 독자가 실제로 겪는 장면을 현장어로 1줄. 마케팅 용어(전환율·퍼널·고객 의도) 금지. "광고비는 그대로인데 전화가 줄었다" 같은 장면.
...
[BAD] core_angle "AI 도구 활용한 최적화가 중요" / evidence_needed ["사례","트렌드"] — 어디 붙여도 맞는 말.
[GOOD] core_angle "AI 가 초안을 쓰는 지금, 마케터의 값어치는 어떤 초안을 버릴지 아는 판단에서 나온다" / evidence_needed ["AI 카피 채택률을 공개한 브랜드 수치","버려진 초안 vs 채택 초안 비교 사례","마케터가 실제로 거른 기준 워크플로우"] — 이 글만 할 수 있는 말 + 찾을 게 명확.

출력은 JSON 만:
{"briefs":[{"id":"<원본 id 그대로>","topic_title":"...","reader_problem":"...","core_angle":"...","hook_candidate":"...","evidence_needed":["...","..."]}]}`;
