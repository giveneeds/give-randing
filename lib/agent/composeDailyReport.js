...
export async function composeDailyReport({ jobId }) {
  ...
  // LLM 보고서용 candidate 요약 — JSON 으로 줘서 LLM 이 자연어로 구성.
  ...
  // 후보별 메타 캐시 (세션 행에 저장 — 나중에 사용자 답변 파싱 시 참조).
...
2) 후보 요약: 강한 것 위주로 2~4개. 각 후보의 id 를 [후보 1] [후보 2] 식으로 라벨링.
   - 무엇을 다루는 자료인지
...
규칙:
- 모든 후보 id 를 본문에 [후보 N] (id: <uuid>) 형태로 표기. 정욱님 답에서 매칭하기 위해 id 노출 필수.
