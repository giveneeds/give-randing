...
function composeCompletionMessage({ item, theme, deepResearch, supplementalResearch, toneResearch, draftId, draftResult, candidatesSummary, jobId }) {
  ...
  // 허브 = 같은 job 의 모든 채택 후보 + 각 후보의 7 variants 한 화면.
  ...
  if (toneResearch?.insights?.voice_patterns?.length) {
    lines.push('', '3차 말투 조정 반영 — 한국 Threads 문장 감각:');
