...
export async function finishPlanningSession({ sessionId, selectedItemId, variantCount = 7 }) {
  ...
  // 스레드 드래프트 생성 — 채택 콘셉트 + freeze 된 findings + 리서치 보강.
  let draftId;
  ...
  try {
    draftResult = await convertItemToThreadDraft({
      ...
      extraContext: buildExtraContext({ creativeBrief, findingsSnapshot, deepResearch, supplementalResearch, toneResearch }),
...
function buildExtraContext({ creativeBrief, findingsSnapshot, deepResearch, supplementalResearch, toneResearch }) {
  ...
  if (!hasCreativeBrief && !hasSnapshot && !deepResearch?.insights && !supplementalResearch?.insights && !toneResearch?.insights) return null;
