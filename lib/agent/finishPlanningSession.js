...
export async function finishPlanningSession({ sessionId, selectedItemId, variantCount = 7 }) {
  ...
  try {
    draftResult = await convertItemToThreadDraft({
      ...
      variantCount: targetVariantCount,
      variantOutlines, // 별도 데이터로도 전달 — per-variant 톤 매칭에 structure_template 사용
    });
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        research_context_used: {
          ...
          variant_tone_samples: draftResult?.variantToneSampleMeta || null,
        },
        auto_generated: true,
      })
...
function buildExtraContext({ creativeBrief, findingsSnapshot, variantOutlines, deepResearch, supplementalResearch, toneResearch }) {
  ...
  if (hasCreativeBrief) {
    lines.push('[채택된 콘셉트 + 기획 — 이 글의 출발점과 독자에게 일으킬 변화. 여기서 벗어나지 말 것]');
    ...
    // 기획 3필드 — 글이 독자에게 일으킬 변화. Writer 가 본문에 반드시 운반.
    if (Array.isArray(creativeBrief.planning_purpose) && creativeBrief.planning_purpose.length) {
      lines.push(`- 기획 목적(planning_purpose): ${creativeBrief.planning_purpose.join(' + ')} (change=관점/기준 바꾸기, resolve=문제 풀기, improve=지금 하는 걸 더 잘하게)`);
    }
    if (creativeBrief.reader_takeaway) {
      lines.push(`- 독자 takeaway(읽고 머릿속에 남는 것 — 본문에 반드시 운반): ${creativeBrief.reader_takeaway}`);
    }
    if (Array.isArray(creativeBrief.proof_anchor_type) && creativeBrief.proof_anchor_type.length) {
      lines.push(`- 증거 형태(proof_anchor_type): ${creativeBrief.proof_anchor_type.join(' + ')}`);
    }
    lines.push('');
  }
  ...
  if (hasSnapshot) {
    ...
    for (const it of findingsSnapshot) {
      const tail = [];
      if (typeof it.tactical_concreteness === 'number') tail.push(`전술점수 ${it.tactical_concreteness}/5`);
      if (Array.isArray(it.specifics) && it.specifics.length) tail.push(`구체: ${it.specifics.join(', ')}`);
...
function buildFindingsSnapshot({ deepResearch, supplementalResearch }) {
  ...
  for (const p of Array.isArray(supp.evidence_points) ? supp.evidence_points : []) {
    ...
    if (!text) continue;
    const tactical = Number.isFinite(Number(p?.tactical_concreteness)) ? Math.round(Number(p.tactical_concreteness)) : null;
    items.push({
      id: next(),
      source: 'supplemental',
      type: 'evidence',
      ...
      tactical_concreteness: tactical,
  ...
  if (px && !px.skipped) {
    for (const k of Array.isArray(px.key_findings) ? px.key_findings : []) {
      // 신구 schema 지원: string 또는 {text, tactical_concreteness, source_url, specifics}
      if (typeof k === 'string') {
        const t = clean(k);
        if (t) items.push({ id: next(), source: 'perplexity', type: 'finding', text: t, tactical_concreteness: null });
      } else if (k && typeof k === 'object') {
        const t = clean(k.text);
        if (!t) continue;
        const tactical = Number.isFinite(Number(k.tactical_concreteness)) ? Math.round(Number(k.tactical_concreteness)) : null;
        items.push({
          id: next(),
          source: 'perplexity',
          type: 'finding',
          text: t,
          citation: clean(k.source_url) || null,
          tactical_concreteness: tactical,
          specifics: Array.isArray(k.specifics) ? k.specifics : [],
        });
      }
    }
  }

  // 점수 있는 finding 우선 정렬. 점수 없는 deep_research 항목들은 뒤에 붙음.
  items.sort((a, b) => {
    const sa = (a.tactical_concreteness ?? -1);
    const sb = (b.tactical_concreteness ?? -1);
    return sb - sa;
  });
