...
//   3) convertItemToThreadDraft — 1차 리서치 + 2차 깊이 리서치 모두 컨텍스트로
//      (deepResearch 결과는 thread_drafts.research_context_used 에 저장)
//   4) 폐기된 후보들 메타 기록 (rejected_candidates)
//   5) 거버넌스 적용 내역 기록 (governance_applied — 어느 KB 가 주입됐는지 메타)
//   6) 마무리 보고 텍스트 작성 + 텔레그램 발송 + 세션 completed
//
  ...
  await supabaseAdmin
    ...
    .update({
      ...
      research_context_used: {
        ...
        generation_decision: draftResult?.generationDecision || existingResearchContext.generation_decision || null,
        variant_review: draftResult?.variantReview || existingResearchContext.variant_review || null,
      },
      auto_generated: true,
    })
    .eq('id', draftId);
