...
export async function finishPlanningSession({ sessionId, selectedItemId, variantCount = 7 }) {
  ...
  // 스레드 드래프트 생성 — 채택 콘셉트 + 1차/2차/보강/말투 리서치 모두 주입.
  let draftId;
  ...
  try {
    draftResult = await convertItemToThreadDraft({
      ...
      extraContext: buildExtraContext({ creativeBrief, deepResearch, supplementalResearch, toneResearch }),
...
function buildExtraContext({ creativeBrief, deepResearch, supplementalResearch, toneResearch }) {
  const hasCreativeBrief = creativeBrief && (creativeBrief.topic_title || creativeBrief.core_angle || creativeBrief.reader_problem);
  if (!hasCreativeBrief && !deepResearch?.insights && !supplementalResearch?.insights && !toneResearch?.insights) return null;
