...
export async function parseUserDecision({ sessionId, userText, candidatesSummary }) {
  ...
  const candidatesArr = Object.values(candidatesSummary || {})
    .map((c) => ({
      ...
      selected_content_pillar: c.selected_content_pillar,
      selected_content_pillar_label: c.selected_content_pillar_label,
      content_pillar_reason: c.content_pillar_reason,
      writing_direction: c.writing_direction,
    }))
...
- "request_research": 추가 리서치 요청 또는 다른 방향/기둥 재평가 요청. ("다른 주제 없어?", "추가 리서치", "후킹 더 살려봐", "이 기둥 말고 뉴스성으로", "실행형 말고 관찰형" 등)
