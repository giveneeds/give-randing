...
function clampTelegramText(text) {
  if (!text || text.length <= TELEGRAM_TEXT_LIMIT) return text;
  return `${text.slice(0, TELEGRAM_TEXT_LIMIT)}\n\n…`;
}

function compactError(message) {
  return String(message || '알 수 없는 오류').slice(0, 500);
}
 ...
 *   telegram_errors?: Array<{item_id: string, source: string, chat_id: string | number, error: string}>,
...
export async function sendItemCards({ jobId, minFitScore = 0.3 }) {
  ...
  if (!items || items.length === 0) {
    return {
      ...
      failed_items: 0,
      skipped_low_fit: 0,
      skipped_reason: '신규 아이템 없음',
    };
  }

  const { data: recipients, error: recipientsError } = await supabaseAdmin
    .from('agent_telegram_recipients')
    .select('chat_id, username, display_name')
    .eq('active', true);
  ...
  for (const item of items) {
    ...
    attemptedItems += 1;
    const cardText = clampTelegramText(buildItemCard(item));
    ...
    for (const r of recipients) {
      try {
        ...
        const error = compactError(e.message);
        telegramErrors.push({
          item_id: item.id,
          source: item.source,
          chat_id: r.chat_id,
        ...
        console.error(`카드 발송 실패 item=${item.id} chat=${r.chat_id}:`, error);
      }
    }
  ...
  if (telegramErrors.length > 0) {
    result.telegram_errors = telegramErrors.slice(0, 10);
  }
  return result;
}
