// 2차 깊이 리서치 — 사용자가 채택한 자료의 주제·앵글에 대해 SNS 후킹 반응 패턴을 한 번 더 본다.
// 1차 리서치(theme_research_snapshots) 가 "이 주제 시장 페인포인트" 라면, 2차는
// "이 콘텐츠 앵글이 SNS 에서 어떻게 반응 받는가" 를 본다. 결과는 스레드 드래프트 생성기에 컨텍스트로 주입.
...
export async function runDeepResearch({ item, theme }) {
  ...
  for (const target of REQUIRED_SOCIAL_SEARCHES) {
    const q = `${queries[0] || title || angle} ${target.label === 'reddit' ? 'reddit discussion' : 'X 반응 OR twitter 반응'}`;
    try {
...
async function extractDeepInsights({ title, angle, persona, results }) {
  ...
  const sys = `너는 기브니즈 콘텐츠 작가의 SNS 리서처다. 채택된 자료/앵글이 Threads·Reddit·X 에서 어떻게 반응을 받는지 패턴을 추출한다.

추출 항목:
- hook_patterns: SNS 인기 게시글에서 자주 보이는 후킹 패턴. 4개 이내. {pattern, example} 형태.
- audience_reactions: 이 주제에 사람들이 자주 보이는 반응·반박·궁금증. 4개 이내. 짧은 문장.
- adapted_angles: 위 결과를 반영해 우리 자료를 Threads 콘텐츠로 발행할 때 권장하는 앵글 2~3개.
