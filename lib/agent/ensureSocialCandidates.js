  ...
  for (const target of SOCIAL_TARGETS) {
    try {
      ...
      const enriched = await enrichItem({
        ...
        sourceLabel: `${target.label} 해외 주제 후보 / ${query}`,
        researchContext,
        themeHint: theme ? {
          name: theme.name,
...
function buildTargetQuery({ target, seeds }) {
  ...
  const suffix = target.source === 'x_search'
    ? 'AI marketing OR marketing trend OR creator tools OR brand campaign OR startup growth'
    : 'AI marketing OR small business marketing OR marketing automation OR creator tools OR growth strategy';
