...
import { buildKnowledgeContext, getThreadsRealBodySamples, getThreadsBadExamples } from '@/lib/knowledge/loader';
 ...
 * @param {{
 *   itemId: string,
 *   formatTypeHint?: string,
 *   extraContext?: string|null,
 *   variantCount?: number,
 *   variantOutlines?: Array<{variant_id:number, structure_template:string, slots:Array<object>}>,
 * }} args
 * @returns {Promise<{ draftId: string, draftIds: string[], postCount: number, model: string }>}
 */
export async function convertItemToThreadDraft({ itemId, formatTypeHint, extraContext, variantCount = 7, variantOutlines = [] }) {
  ...
  // BAD 예시 1개 — AI 말투 회귀 ("Writer 의 기본값") 을 sys 에 박아 GOOD(톤 샘플) 과 비교 강제.
  // 모든 variant 가 같은 BAD 를 받음 — 기본값은 1개로 충분, GOOD 만 variant 별 다름.
  const badExamples = getThreadsBadExamples({ maxSamples: 1 });
  const badExampleBlock = badExamples.length > 0
    ? `[BAD — 이게 너의 기본 AI 말투다. 이 결로 쓰면 실패.]\n${badExamples[0].body}\n↑ 진단: ${badExamples[0].diagnosis}`
    : '';
  ...
  // variant 마다 *다른 톤 샘플* 받게 — variantHint(hook/intent + outline 의 structure_template) 로 매칭.
  // excludeRootTexts 누적으로 같은 샘플이 여러 variant 에 중복되는 것 막음.
  // 톤 매칭 결과는 sharedToneReferenceQuery 외에 variantHint 가중치(hook +2 / intent +1.5 / structure +1) 합산.
  const sharedToneReferenceQuery = [
    theme?.name,
    brief.recommended_title,
    brief.content_angle,
    Array.isArray(brief.content_angles) ? brief.content_angles.join(' ') : '',
  ...
  const outlineByVariantId = new Map(
    (Array.isArray(variantOutlines) ? variantOutlines : []).map((o) => [Number(o?.variant_id), o])
  );
  const usedSampleRootTexts = [];
  const variantToneSampleMeta = []; // 검수용 — 어떤 샘플이 어느 variant 에 갔나
  ...
  // 톤 샘플 페치는 *순차* 로 미리 해서 excludeRootTexts 누적이 의미를 갖게 한다.
  const variantTasks = [];
  for (let i = 0; i < targetVariantCount; i += 1) {
    const variantId = i + 1;
    const assignedHook = HOOK_ROTATION[i % HOOK_ROTATION.length];
    const assignedIntent = intentRotation[i % intentRotation.length];
    const outline = outlineByVariantId.get(variantId) || null;
    const structureTemplate = outline?.structure_template || null;
    // variantHint 로 톤 샘플 1개 픽 — maxSamples=1 로 강하게 1개씩 분산. excludeRootTexts 로 중복 방지.
    const toneRes = getThreadsRealBodySamples({
      queryText: sharedToneReferenceQuery,
      maxSamples: 1,
      variantHint: { hook_pattern: assignedHook, engagement_intent: assignedIntent, structure_template: structureTemplate },
      excludeRootTexts: usedSampleRootTexts,
      debug: true,
    });
    const variantToneBlock = toneRes?.block || '';
    const variantTonePick = (toneRes?.picks || [])[0] || null;
    if (variantTonePick?.rootText) usedSampleRootTexts.push(variantTonePick.rootText);
    variantToneSampleMeta.push({
      variant_id: variantId,
      hook: assignedHook,
      intent: assignedIntent,
      structure: structureTemplate,
      picked_tone_meta: variantTonePick?.tone_meta || null,
      picked_topic_label: variantTonePick?.topic_label || null,
      picked_root_preview: (variantTonePick?.rootText || '').slice(0, 60),
      score: variantTonePick?.score || 0,
    });
    variantTasks.push(generateSingleVariant({
      ...
      model,
      variantToneBlock, // variant 전용 톤 샘플 1개
      ...sharedContext,
    }));
  }
  ...
  // variant_id → 그 variant 에 주입된 톤 샘플 메타 매핑 — risk_flag 측정에 사용.
  const toneMetaByVariantId = new Map(
    variantToneSampleMeta.map((m) => [Number(m.variant_id), m.picked_tone_meta || null])
  );
  const insertRows = variantsToSave.map((variant) => {
    ...
    const expectedToneMeta = toneMetaByVariantId.get(Number(variant.variant_id)) || null;
    const riskFlags = buildRiskFlags(variant, generationDecision, posts, expectedToneMeta);
    const toneMetrics = measureToneMetrics(posts);
    ...
    return {
      ...
      research_context_used: {
        ...
        expected_tone_meta: expectedToneMeta,
        variant_review: {
  ...
  return {
    ...
    variantToneSampleMeta, // 검수용 — 어느 톤 샘플이 어느 variant 에 매칭됐는지
  };
}
...
async function generateSingleVariant({
  ...
  const sys = `${ontologyPrefix({ includeEnums: false })}[CRITICAL — 수치·고유명사·인용 규칙 (위반 시 variant 폐기 위험)]
- 본문에 수치(%, 배수, "N명", "N만원" 등), 실제 브랜드·제품·도구·인물 고유명사, 직접 인용("~라고 했다")을 쓰려면 아래 [FINDINGS SNAPSHOT] 안의 finding(F001/F002…) 가 그 사실을 직접 명시하고 있어야 한다.
- snapshot 에 없는 수치는 절대 만들지 마라. 학습된 그럴듯한 숫자("매출 15% 상승", "30% 감소")도 끌어다 쓰지 마라. 출처 없으면 곧 변종 폐기 사유다.
- snapshot 에 없는 브랜드명·제품명·도구명·고유명사도 본문에 넣지 마라. ("코카콜라가 ~했다" 처럼 snapshot 에 없으면 금지.)
- 슬롯이 비면 "수치는 공개되지 않음", "정확한 비교는 어렵다" 같은 솔직한 표현 또는 일반화("잘하는 곳은 조용히 바꾸고 있다") 로 대체한다.
- 단, 사회 일반 상식(예: "AI 도구가 늘었다") 은 수치·고유명사 없이 쓸 수 있다. 금지되는 건 *구체 수치·실제 고유명사·구체 인용* 이다.
- 검증 가능한 finding 을 본문에 그대로 베끼지 말고 reader_problem 의 호흡으로 풀어 써라. 단, 사실 자체는 흐트리지 마라.
- 본문에는 [F001], [F011, F015] 같은 finding id 라벨을 절대 적지 마라. id 는 너만 머릿속으로 확인하고, 본문에는 사실만 자연스럽게 녹여 써라. 출처가 필요하면 "한 LinkedIn 기사에서는~", "한 업계 자료에서는~" 같이 풀어서 표현하라(brand/site 이름은 snapshot 의 citation 에 있을 때만).

너는 기브니즈(B2B 마케팅 에이전시)의 Threads 콘텐츠 작가다.
...
이번 호출은 후보 1개만 만든다. variant_id = ${variantId}.

[독자에게 무엇을 일으키는가 — 본문에 반드시 운반]
extraContext 의 [채택된 콘셉트 + 기획] 블록에 "독자 takeaway(읽고 머릿속에 남는 것)" 가 있으면 그 문장이 *글의 마지막 1~2줄에서 자연어로 명시*되어야 한다. 라벨("행동:" / "관점:" / "기준:") 은 본문에 그대로 노출하지 말고, 그 *내용* 만 자연어로. 예: takeaway 가 "행동: 광고 켜기 전 랜딩 첫 화면이 광고 카피와 같은 말을 하는지 한 줄로 점검한다" 면, 마무리에 "광고 켜기 전에 / 랜딩 첫 화면 한 줄만 다시 본다 / 카피랑 같은 말 하는지" 같이 풀어 쓴다.
planning_purpose 가 "change" 면 글이 *독자 관점을 바꾸는 끝맺음* 으로, "resolve" 면 *문제 해결의 다음 한 발* 로, "improve" 면 *지금 하는 걸 한 단계 더 가는 방향* 으로 닫는다.
proof_anchor_type 이 "numbers" 면 본문에 수치 1개+ (snapshot 출처), "case" 면 구체 사례 1개+, "workflow" 면 단계 나열 1개+, "comparison" 면 정보 vs 기준 / Before vs After 대비 1개+ 가 보여야 한다.
...
[ARCHITECT OUTLINE — 반드시 본문 형식에 반영]
extraContext 의 "[ARCHITECT OUTLINES — ...]" 블록에서 "variant ${variantId}" 항목을 찾아 그 outline 을 다음과 같이 *형식 자체에 반영*한다:

- structure_template = "info_vs_criterion" → 본문에 실제로 "1) 정보: <한 줄> / 기준: <한 줄> / 2) 정보: ... / 기준: ... / 3) ..." 의 1)2)3) 대비 줄이 보여야 한다. 이 형식이 없으면 실패.
- structure_template = "command_to_output" → "❶ <스텝1> / ❷ <스텝2> / ❸ <스텝3>" 처럼 번호 기호로 워크플로우가 보여야 한다. 일반 산문이면 실패.
- structure_template = "before_after" → "이전: <장면> → 이후: <장면+수치>" 처럼 before/after 가 명확히 구분되어 보여야 한다.
- structure_template = "numbers_then_meaning" → 첫 줄(hook)이 수치로 시작해야 한다 (예: "44%." "2.5배."). 수치가 첫 줄에 없으면 실패.
- structure_template = "counter_argument" → "예상 반론: ~ / 사실은: ~" 형식이 보여야 한다.
- structure_template = "observation_breaks_norm" → "관찰 → 통념 → 그런데 사실은" 흐름이 보여야 한다.
- structure_template = "question_engagement" → 마지막 줄이 독자에게 묻는 열린 질문이어야 한다.

각 슬롯의 role 순서대로 본문 흐름이 흐른다. 슬롯의 takeaway 가 그 슬롯이 끝났을 때 독자 머릿속에 남는 한 줄이어야 한다. 슬롯의 evidence_ids 에 핀 박힌 F### 의 사실(수치·고유명사 포함)을 그 슬롯 본문에 자연어로 녹여 넣는다.

[수치 핀 박기 — 매우 중요] outline 슬롯의 evidence_ids 에 F010·F017 처럼 수치(%·배·명·만원 등)를 포함한 finding 이 있으면, 그 슬롯 본문에 그 수치를 *반드시* 박는다. 수치가 박힐 슬롯에서 빠뜨리면 실패 — 그 슬롯의 핵심이다.

[ID 라벨 절대 금지] 본문에 [F001], [F011, F015] 같은 ID 라벨 표기를 절대 적지 마라. ID 는 너만 머릿속으로 확인하고, 본문에는 사실만 자연어로 녹여라. "한 LinkedIn 기사에서는~", "한 업계 자료에서는~" 처럼 풀어서 표현. ID 가 본문에 1번이라도 보이면 그 variant 는 실패다.

단, 위에 지정된 hook_pattern·engagement_intent·content_pillar 와 outline 의 값이 다르면 위 지정값을 우선한다(rotation 호환성).
...
${badExampleBlock ? `${badExampleBlock}\n\n` : ''}${variantToneBlock ? `[GOOD — 이 variant 전용 톤 샘플. 위 BAD 를 깨고 *이 호흡* 으로 써라.]\n${variantToneBlock}\n\n[톤 목표 — variantHint 의 tone_meta 기준]\n- 문장 평균 길이: 톤 샘플이 *짧은(short)* 호흡이면 평균 15~22자, *중간(mid)* 이면 18~28자, *긴(long)* 이면 25~35자. 톤 샘플 호흡과 다르면 실패.\n- 종결 다양성: 같은 종결(예: "~합니다" / "~할까요?")이 본문의 50%를 넘으면 실패. "~다 / ~지 / ~네 / ~잖아 / ~죠 / ~어요 / ~군요" 등 섞어라.\n- "?" 개수: 본문 전체 2개+. 단, 마지막 줄을 "어떻게 활용하고 계신가요?" / "여러분의 팀은 어떤~" / "어떤 변화가~" 류 *어디 붙여도 되는 일반 질문* 으로 닫으면 실패.\n- register: 톤 샘플 register 가 *ban(반말)* 이면 본문에 반말 단서(앎./함./임./이김/없음/ㅋ 같은 어미 또는 추임) 최소 1개 등장. *jon(존댓말)* 이면 단정문(~다.) 과 청유/질문(~죠?·~네요) 섞어라. *mixed* 면 ban·jon 둘 다 등장.\n- 줄바꿈: 같은 길이의 줄(±3자) 5개+ 연속이면 실패. 일부러 들쭉날쭉.\n- 이모지: 톤 샘플에 이모지가 있으면 0~1개 허용, 없으면 0개.\n- "AI는 도구일 뿐" "결국 본질은 사람" 같은 어디 붙여도 맞는 결론 마무리는 절대 금지(이미 [BAD] 진단).\n\n위 목표를 *반드시* 따른다. 본문이 [BAD] 의 진단 패턴(존댓말 일관·평균 30자+·일반화 질문 마감) 에 가까우면 risk_flags 에 "문체 평균 회귀" 와 "톤 샘플 미반영" 둘 다 추가하고 다시 써라.\n\n` : ''}${knowledgeBlock ? `[참조 지식·거버넌스]\n${knowledgeBlock}\n\n` : ''}출력 형식: JSON 만. 후보 1개 객체만 반환한다(variants 배열·scores 없음). 형식:
{
...
function sanitizePosts(postsArr) {
  return (Array.isArray(postsArr) ? postsArr : []).slice(0, 8).map((p, i) => {
    ...
    // findings_id 라벨([F001], [F011, F015] 등) 본문 노출 차단. 프롬프트로 막아도 LLM 이 자주 위반.
    // 라벨 앞뒤 공백/줄바꿈도 같이 정리해 자연스러운 본문 유지.
    const cleaned = raw
      .replace(/\s*\[F\d{3}(?:\s*,\s*F\d{3})*\]\s*/g, ' ')
      .replace(/[ \t]{2,}/g, ' ')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
    const sliced = cleaned.slice(0, 1000);
    return {
...
// 본문 톤 자동 측정 — Writer 자가점검(거짓 보고 가능)에 의존하지 않고 코드로 객관 측정.
// 결과는 buildRiskFlags 가 받아 risk_flags 자동 추가에 사용.
const BAN_REGISTER_HINTS = /(앎\?|함\.|임\.|이김|놨|봐봐|줘봐|이거지|었네|이지|던데|이야\.|되네|되지|이거임|있음|없음|진다\.|지|네\.|잖아|었지|어딨|싶어|그래|이쟈|이러)/g;
const JON_REGISTER_HINTS = /(니다|어요|아요|시나요|일까요|입니다|했어요|이에요|예요|네요|하죠|되죠)/g;
const GENERIC_QUESTION_TAIL = /(어떻게\s*활용하고\s*계신가요|여러분의\s*팀은\s*어떤|어떤\s*변화가\s*있을까요|준비\s*되었나요|어떤\s*역량이|어떤\s*전략이|어떻게\s*생각하시나요)/;
function measureToneMetrics(posts) {
  const full = posts.map((p) => p.body || '').join('\n');
  if (!full.trim()) {
    return { avg_sentence_chars: 0, question_count: 0, ban_hits: 0, jon_hits: 0, generic_tail: false, repeat_endings: false, lines: 0 };
  }
  const sentences = full.split(/[\n.!?]/).map((s) => s.trim()).filter(Boolean);
  const avg = sentences.length ? sentences.reduce((s, x) => s + x.length, 0) / sentences.length : 0;
  const banHits = (full.match(BAN_REGISTER_HINTS) || []).length;
  const jonHits = (full.match(JON_REGISTER_HINTS) || []).length;
  const qCount = (full.match(/\?/g) || []).length;
  // 종결 반복 — 마지막 3줄 중 2개 이상이 같은 어미면 단조롭다 판단.
  const tailLines = full.split('\n').map((l) => l.trim()).filter(Boolean).slice(-3);
  const tailEndings = tailLines.map((l) => (l.match(/[가-힣]{2,3}[.?!]?$/) || [''])[0]);
  const repeatEndings = tailEndings.length >= 2 && tailEndings.filter((e) => e === tailEndings[tailEndings.length - 1]).length >= 2;
  const genericTail = GENERIC_QUESTION_TAIL.test(tailLines.join(' '));
  return {
    avg_sentence_chars: Math.round(avg),
    question_count: qCount,
    ban_hits: banHits,
    jon_hits: jonHits,
    generic_tail: genericTail,
    repeat_endings: repeatEndings,
    lines: full.split('\n').filter((l) => l.trim()).length,
  };
}

// tone_meta 와 측정값을 비교해 risk_flags 후보를 만든다. buildRiskFlags 에서 합쳐 변종 메타에 박는다.
function deriveToneRiskFlags(metrics, expectedToneMeta) {
  const flags = [];
  if (!metrics) return flags;
  // 회귀 시그널 — 평균 문장 길고 + 반말 0 + 일반화 질문 마감.
  if (metrics.avg_sentence_chars >= 30 && metrics.ban_hits === 0 && metrics.generic_tail) {
    flags.push('문체 평균 회귀(존댓말 일관·평균 30자+·일반화 마감)');
  }
  // 종결 단조.
  if (metrics.repeat_endings) {
    flags.push('종결 다양성 부족(같은 어미 반복)');
  }
  // 일반화 마감 단독.
  if (metrics.generic_tail) {
    flags.push('어디 붙여도 맞는 일반화 질문으로 마감');
  }
  // register 불일치 — 톤 샘플이 ban 인데 ban_hits 0 + jon_hits 다수.
  if (expectedToneMeta?.register === 'ban' && metrics.ban_hits === 0 && metrics.jon_hits >= 3) {
    flags.push(`톤 샘플 미반영(샘플 register=ban 인데 본문은 존댓말 ${metrics.jon_hits}회·반말 0)`);
  }
  // 짧은 호흡 기대인데 평균 30자+.
  if (expectedToneMeta?.rhythm === 'short' && metrics.avg_sentence_chars >= 30) {
    flags.push(`톤 샘플 미반영(샘플 rhythm=short 인데 평균 ${metrics.avg_sentence_chars}자)`);
  }
  return flags;
}
...
function buildRiskFlags(variant, generationDecision, posts, expectedToneMeta = null) {
  ...
  // 톤 측정 기반 자동 risk_flag — Writer 의 자가 점검에 의존하지 않고 코드로 객관 측정.
  const toneMetrics = measureToneMetrics(posts);
  const toneFlags = deriveToneRiskFlags(toneMetrics, expectedToneMeta);
  for (const f of toneFlags) {
