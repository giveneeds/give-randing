...
export async function convertItemToThreadDraft({ itemId, formatTypeHint, extraContext, variantCount = 7 }) {
  ...
  for (let i = 0; i < targetVariantCount; i += 1) {
    ...
    const assignedPillar = pillarRotation.length > 0 ? pillarRotation[i % pillarRotation.length] : null;
    ...
    priorVariants.push(summarizeVariantForDedup(result.variant));
  }
...
- 한 포스트는 최대 1000자다. single_post 는 반드시 500자 이상으로 쓴다. 정보형 연결글은 전체 본문 합계 800자 이상으로 쓴다. 연결글의 1번 포스트는 80~220자의 hook 을 우선하고, 2번 이후 포스트에서 정보 밀도를 채운다.
...
- 출력은 후보 1개 JSON 객체만.`;
  ...
  if (hardIssues.length > 0) {
    try {
      const retryUser = `${user}\n\n[직전 시도 문제]\n${hardIssues.join(' / ')}\n위 문제를 고쳐서 다시 후보 1개를 만든다. single_post 는 500자 이상, 정보형 연결글은 총 800자 이상, 후속 포스트는 각 250자 이상으로 쓴다.`;
