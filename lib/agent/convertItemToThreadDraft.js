...
const PROMPT_VERSION = 'thread_v2_density_2026_05_23';
...
- 본문 작성 전에 density_check 를 채운다. practical_examples 가 3개 미만이거나 old_friction/new_change/giveneeds_angle 중 하나가 비면 research_sufficiency='thin' 으로 둔다.
...
- 출력은 JSON 만.`;
  ...
  const generationDecision = buildGenerationDecision(parsed, densityCheck);
  const riskFlags = Array.isArray(parsed.risk_flags) ? parsed.risk_flags.filter((x) => typeof x === 'string') : [];
...
function buildGenerationDecision(parsed, densityCheck) {
  ...
  if (missingRequired.length > 0 || groundedExamples.length < 3) {
    researchSufficiency = researchSufficiency === 'missing' ? 'missing' : 'thin';
  }

  return {
    content_pillar: typeof parsed.content_pillar === 'string' ? parsed.content_pillar : null,
