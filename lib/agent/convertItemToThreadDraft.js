...
- Threads는 한 포스트에 최대 1000자. single_post 는 250~600자로, 짧은 문장(20자 이내) 여러 개를 줄바꿈으로 쌓아 한 포스트 안에서 도입→전개→마무리 흐름을 만든다. 한 문장만 길게 늘이지 않는다.
...
- 문장 길이를 일부러 들쭉날쭉하게 쓴다. 5자로 툭 끊기도 하고, 20자로 흘러가다, 가끔 40자로 한 번에 풀기도 한다. 사람이 말하듯 호흡과 완급이 느껴져야 한다.
- 모든 문장이 비슷한 길이(예: 다 30자 안팎)면 실패다. 짧은 문장을 더 많이 쓰되, 길이를 규칙적으로 만들지 말고 변칙적으로 섞는다. 줄바꿈을 자주 써서 빠르게 읽히게 한다.
...
- FOMO는 글 안에 녹이는 심리 장치다. 약하게 흉내만 내지 말고 분명하게 만든다. fomo_intensity 는 기본 clear 로 두고, 첫 포스트나 마무리에서 "놓침·격차·뒤처짐" 감정 하나를 선명하게 건드린다. 단 "망합니다", "끝입니다", "무조건 해야 합니다" 같은 과격한 공포 문장은 여전히 금지 — 공포가 아니라 "나만 놓치는 것 같은 감각" 으로.
...
- 고백/경험 서사를 쓸 때 에이전트의 1인칭 경험을 지어내지 않는다. 수집 자료의 실제 사례를 "어떤 사장님이…", "어떤 AI 전문가가…", "해외 사례에선 이렇게 쓰더라구요…" 같은 3인칭 관찰로만 인용한다. "저도 ~해봤습니다" 같은 봇의 가짜 1인칭은 금지.
...
- confession_story(고백 서사)는 반드시 3인칭 관찰로만 쓴다. "저도 ~해봤습니다" 같은 봇의 가짜 1인칭은 절대 금지다.
...
- 포스트 수는 주제의 밀도로 결정한다. single_post 는 한 포스트 안에서 250~600자로 장면, 맥락, 관점, 마무리가 짧은 문장 여러 개로 서야 한다. short_thread 는 각 포스트 역할이 분리될 때만 쓴다. resource_thread 는 실제 예시/도구/문장/검색어가 5개 이상일 때만 쓴다.
...
- 한 포스트는 최대 1000자다. single_post 는 250~600자로 쓰되 문장은 20자 이내로 짧게 끊는다. 정보형 연결글은 전체 본문 합계 800자 이상으로 쓴다. 연결글의 1번 포스트는 80~220자의 hook 을 우선하고, 2번 이후 포스트에서 정보 밀도를 채운다.
...
function buildPerVariantReview({ variants, targetVariantCount, pillarCandidates }) {
  ...
  // 추천 = risk_flags 가 가장 적은 후보(동률이면 첫 번째).
