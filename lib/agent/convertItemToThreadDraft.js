...
// 겹치지 않는 7개 방향을 만들고, 자동 채점하되 모든 후보를 저장한다.
...
주 독자는 브랜드·마케팅을 운영하는 사람 전체다. 원문/브리프가 특정 업종을 말할 때만 요식업 사장님·병의원 원장·자영업자로 좁힌다. 뉴스성/시사성 소재는 억지로 "자영업자 실무팁"으로 닫지 않아도 된다.

규칙:
- Threads는 한 포스트에 최대 1000자까지 쓸 수 있다. single_post 는 최소 500자 이상, 권장 500~900자로 쓴다. 연결글의 각 포스트는 역할에 따라 350~850자를 권장한다.
...
- "자영업자 여러분", "사장님들도" 같은 직접 호명은 brief.suggested_persona 나 theme.target_persona 가 그 대상을 명확히 가리킬 때만 쓴다. 아니면 "브랜드를 운영하는 사람", "마케팅을 만드는 사람", "콘텐츠를 올리는 사람"처럼 넓게 쓴다.
...
- 한 번에 서로 겹치지 않는 7개 후보를 만든 뒤 채점한다. 최종 선택은 사용자가 한다. 너는 recommended_variant_id 만 제안하고, 7개 후보를 모두 보존한다.
- 7개 후보는 위에서 고른 2~3개 pillar 후보 안에서 나뉘어야 한다. 후보끼리는 content_pillar, content_treatment, hook, 문장 리듬, 결론 방식이 서로 달라야 한다. 같은 말을 형식만 바꿔 반복하면 실패다.
...
${knowledgeBlock ? `[참조 지식·거버넌스]\n${knowledgeBlock}\n\n` : ''}출력 형식: JSON 만. 형식:
{
  ...
  "recommendation_reason": "7개 중 먼저 검토하면 좋은 후보를 제안한 이유. 최종 선택은 사용자가 한다.",
  "variant_notes": [
...
- 한 포스트는 최대 1000자다. single_post 는 반드시 500자 이상으로 쓴다. 연결글도 각 포스트가 너무 빈약하지 않게 350~850자 범위를 우선한다.
- short_thread 는 각 포스트 역할이 서로 달라야 한다. 같은 주장을 반복하면 format_fit 2점 이하.
- 먼저 content_pillar 후보 2~3개를 점수화해 pillar_candidates 에 담는다. 그 다음 그 기둥 후보들 안에서 서로 겹치지 않는 7개 글 후보를 만든다.
- 7개 후보는 모두 사용자가 볼 수 있는 최종 후보로 작성한다. 내부에서 삭제하거나 1개로 압축하지 않는다.
- 7개 후보는 관점, pillar, treatment, FOMO 녹이는 방식, 설명 방식, 길이, hook, 결론 방식이 달라야 한다.
- 각 후보를 1~5점으로 채점한다. overall 은 평균이 아니라 "실제로 발행해도 어색하지 않은가"를 최우선으로 판단한다.
- 최종 선택은 사용자가 한다. 너는 recommended_variant_id 로 먼저 보면 좋은 후보만 제안하고, 나머지 6개도 모두 보존한다.
- 광고티 X, 실무 적용 가능한 인사이트 중심.
- 전문 용어가 나오면 돈/시간/화면/행동/감정이 보이는 말로 치환한다.
- 첫 2줄에는 타깃이 겪는 실제 상황이나 말투가 보여야 한다.
- 첫 3줄은 실제 사람이 쓰는 말/검색어/장면으로 시작한다. "키워드", "퍼널", "전환", "고객 의도" 같은 분석어로 시작하지 않는다.
...
- 7개 후보 중 최소 5개는 서로 다른 결론을 가져야 한다. 예: 감정 관찰, 맥락 리스크, 내부 검수 관점, 밈/말장난 한계, 사과 전 대응, 소비자 기억, 브랜드 신뢰처럼 아예 다른 처리 방식.
...
- 1,2,3 식 포스트 분리는 실행 단계나 목록 가치가 있을 때만 쓴다. single_post 로 끝낼 때도 500자 이상으로 장면, 맥락, 관점, 마무리를 충분히 담는다.
...
async function reviewAndMaybeRepairVariants({ parsed, originalTitle, originalText, brief, extraContext, itemId, model }) {
  if (!Array.isArray(parsed?.variants) || parsed.variants.length === 0) {
    return { metadata: { skipped: true, reason: 'variants 없음' }, repairedParsed: null };
  }

  const compactVariants = parsed.variants.slice(0, 7).map((v) => ({
...
수정이 필요하면 revised_variants 에 7개 후보 전체를 다시 반환한다. 괜찮으면 revised_variants 는 빈 배열.
출력은 JSON만.`;
...
브리프:
${JSON.stringify({
  persona: brief.suggested_persona,
...
JSON:
{
  ...
  try {
    ...
    const revisedVariants = Array.isArray(review.revised_variants)
      ? review.revised_variants.map(normalizeVariant).filter(Boolean).slice(0, 7)
      : [];

    const repairedParsed = review.needs_revision && revisedVariants.length === parsed.variants.length
    ...
    return {
      ...
      metadata: {
        ...
        variant_feedback: Array.isArray(review.variant_feedback) ? review.variant_feedback.slice(0, 7) : [],
      },
    };
  } catch (e) {
    return {
  ...
  return {
    ...
    metadata: {
      ...
      variant_notes: Array.isArray(parsed.variant_notes)
        ? parsed.variant_notes.slice(0, 7)
