...
- Threads 레퍼런스 구조 벤치마크가 있으면 본문 밀도 판단에 반영한다. 단, 이미지 카드형 레퍼런스의 짧은 캡션은 텍스트 원고의 낮은 기준으로 삼지 않는다. 카드 OCR이 없는 표본은 "이미지 안 정보가 있을 수 있음"으로 보고, 우리 원고는 텍스트만으로도 이해될 만큼 맥락을 보강한다.
...
- 절대 길이 기준: short_thread/resource_thread/mega_thread 중 explainer, practical_tip, checklist, case_note, news_commentary 는 전체 본문 합계 800자 이상을 기본으로 한다. 각 후속 포스트는 ending 전까지 250자 이상을 우선한다. 60~100자짜리 요약 포스트 여러 개는 실패다.
...
- 연결글의 마지막 ending 포스트를 제외한 각 포스트는 실제 한국어 본문 기준 380자 이상으로 쓴다.
...
function buildRiskFlags(variant, generationDecision, posts) {
  ...
  if (posts.length > 1 && posts.some((p, idx) => idx < posts.length - 1 && (p.body?.length || 0) < minimumPostCharsForVariant(variant)) && !riskFlags.includes('후속 포스트 정보 밀도 부족')) {
...
function normalizeVariant(input, idx) {
  ...
  const minPostChars = minimumPostCharsForVariant(tempVariant);
  if (posts.length > 1 && posts.some((p, postIdx) => postIdx < posts.length - 1 && (p.body?.length || 0) < minPostChars)) {
    scores.substance = Math.min(scores.substance, 2);
    scores.format_fit = Math.min(scores.format_fit, 2);
    scores.overall = Math.min(scores.overall, 2);
    scoreReason = `${scoreReason ? `${scoreReason} ` : ''}후속 포스트가 ${minPostChars}자 미만으로 정보 단위가 부족해 감점.`;
  }
...
function collectHardQualityIssues(variants) {
  return (Array.isArray(variants) ? variants : []).flatMap((variant, idx) => {
    ...
    if (posts.length > 1) {
      const minPostChars = minimumPostCharsForVariant(normalized);
      const thinPosts = posts
        .filter((p, postIdx) => postIdx < posts.length - 1 && (p.body?.length || 0) < minPostChars)
