...
import { collectRedditRss } from '@/lib/collectors/redditRss';
import { collectSocialSearch } from '@/lib/collectors/socialSearch';
...
const COLLECTORS = {
  ...
  reddit: (s) => collectRedditRss(s.identifier, s.meta || {}),
  x_search: (s) => collectSocialSearch(s.identifier, { ...(s.meta || {}), source: 'x_search' }),
  reddit_search: (s) => collectSocialSearch(s.identifier, { ...(s.meta || {}), source: 'reddit_search' }),
};
...
export async function runCollection(opts = {}) {
  ...
  try {
    ...
    // 2-1) 주제(content_themes) → source 매핑 로드.
    // collection_source_ids 배열에 src.id 가 포함된 첫 active 주제를 src 의 "기본 주제" 로 사용.
    // 같은 source 가 여러 주제에 묶일 수 있으나 MVP 는 첫 매칭 우선.
    const sourceIdToTheme = new Map();
    const themeInsightsCache = new Map();
    try {
      const { data: themes } = await supabaseAdmin
        .from('content_themes')
        .select('id, name, target_persona, target_topic_cluster, collection_source_ids, active')
        .eq('active', true)
        .order('sort_order', { ascending: true });
      for (const t of themes || []) {
        for (const srcId of t.collection_source_ids || []) {
          if (!sourceIdToTheme.has(srcId)) sourceIdToTheme.set(srcId, t);
        }
      }
    } catch (e) {
      // 주제 매핑 실패는 무시 — theme_id 없이 일반 수집으로 진행.
      errors.push(`themes load: ${e.message}`);
    }
    ...
    for (const src of sources) {
      ...
      const linkedTheme = sourceIdToTheme.get(src.id) || null;
      const queryEntries = buildPillarDrivenSourceQueries(src, { linkedTheme, trigger });
      let items = [];
      for (const entry of queryEntries) {
        try {
          const collected = await fn(entry.source);
          items.push(...collected.map((item) => ({
            ...item,
            raw_data: {
              ...(item.raw_data || {}),
              content_pillar_query: entry.pillar,
              pillar_query_reason: entry.reason,
              original_source_identifier: src.identifier,
              generated_query: entry.query,
            },
          })));
        } catch (e) {
          errors.push(`collect ${src.source_type}/${entry.query || src.identifier}: ${e.message}`);
          stats.failed += 1;
        }
      }
      if (items.length === 0) continue;

      const onTopicItems = items.filter((item) => !isOffTopicCollectedItem(item));
      stats.skipped += items.length - onTopicItems.length;
      items = onTopicItems;
      if (items.length === 0) continue;
      ...
      // 이 src 가 묶인 주제 + 그 주제의 최신 리서치 인사이트(있으면) 미리 로드.
      let researchContext = null;
      if (linkedTheme) {
        if (themeInsightsCache.has(linkedTheme.id)) {
          researchContext = themeInsightsCache.get(linkedTheme.id);
        } else {
          try {
            researchContext = await getLatestInsightsForTheme(linkedTheme.id);
          } catch {
            researchContext = null;
          }
          themeInsightsCache.set(linkedTheme.id, researchContext);
        }
      }
      ...
      for (const item of toProcess) {
        ...
        try {
          enriched = await enrichItem({
            ...
            researchContext,
            themeHint: linkedTheme ? {
              name: linkedTheme.name,
              target_persona: linkedTheme.target_persona,
              target_topic_cluster: linkedTheme.target_topic_cluster,
            } : null,
          });
          stats.enriched += 1;
        } catch (e) {
