...
//
// cron 트리거에서는 src.meta.daily_limit (기본 1) 만큼만 수집해 텔레그램 다이제스트 부담을 줄임.
// 수동 트리거(어드민)는 cap 없이 전량 수집.
...
export async function runCollection(opts = {}) {
  ...
  try {
    ...
    for (const src of sources) {
      ...
      try {
        items = await fn(src);
        // cron 트리거는 매체 부담 방지를 위해 src 당 daily_limit 만큼만 처리(기본 1건).
        // 수동 트리거(어드민)는 cap 없이 전체 처리.
        if (trigger === 'cron') {
          const cap = Number.isFinite(src.meta?.daily_limit) ? Math.max(1, src.meta.daily_limit) : 1;
          items = items.slice(0, cap);
        }
        stats.by_source[src.source_type] = (stats.by_source[src.source_type] || 0) + items.length;
      } catch (e) {
