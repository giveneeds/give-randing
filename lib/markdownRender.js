...
function renderInline(text, keyPrefix = '') {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={k} className="font-extrabold text-inherit">{part.slice(2, -2)}</strong>;
    }
    if (part.startsWith('*') && part.endsWith('*') && part.length > 2) {
      return <em key={k} className="italic text-inherit">{part.slice(1, -1)}</em>;
    }
 ...
 * @param {string} textSize - 'default' | 'sm' | 'md' | 'lg' | 'xl'
 * @param {string} textWeight - 'regular' | 'medium' | 'bold' | 'black'
 * @param {{role?: string, color?: string}} textStyle
 */
export default function MarkdownContent({ text, variant = 'default', textSize = 'default', textWeight = 'medium', textStyle }) {
  if (!text) return null;

  const isDark = variant === 'dark';
  const isCompact = variant === 'compact';
  const hasTextStyle = textStyle && typeof textStyle === 'object';
  const normalizedStyle = normalizeServiceTextStyle(textStyle, { role: 'body', color: isDark ? 'inverse' : 'default' });
  const sizeCls = hasTextStyle
    ? (isCompact && normalizedStyle.role === 'body' ? 'text-xs font-semibold leading-relaxed' : serviceTextRoleClassName(normalizedStyle.role))
    : (isCompact ? 'text-xs' : (TEXT_SIZE_CLASSES[textSize] || TEXT_SIZE_CLASSES.default));
  const weightCls = hasTextStyle ? '' : (TEXT_WEIGHT_CLASSES[textWeight] || TEXT_WEIGHT_CLASSES.medium);

  const bodyCls = hasTextStyle
    ? serviceTextColorClassName(normalizedStyle.color, isDark ? 'dark' : 'default')
    : (isDark ? 'text-zinc-200' : 'text-zinc-600 dark:text-zinc-300');
  const headingCls = hasTextStyle
    ? serviceTextColorClassName(normalizedStyle.color, isDark ? 'dark' : 'default')
  ...
  const spacingCls = isCompact ? 'space-y-3' : 'space-y-4';
  ...
  return (
    <div className={`${sizeCls} ${bodyCls} ${weightCls} leading-relaxed ${spacingCls}`}>
      {blocks.map((block, bi) => {
