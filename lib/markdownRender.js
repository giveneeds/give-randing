 ...
 */
export default function MarkdownContent({ text, variant = 'default', textSize = 'default', textWeight = 'medium' }) {
  if (!text) return null;

  const isDark = variant === 'dark';
  const isCompact = variant === 'compact';
  const sizeCls = isCompact ? 'text-xs' : (TEXT_SIZE_CLASSES[textSize] || TEXT_SIZE_CLASSES.default);
  const weightCls = TEXT_WEIGHT_CLASSES[textWeight] || TEXT_WEIGHT_CLASSES.medium;
  ...
  return (
    <div className={`${sizeCls} ${bodyCls} ${weightCls} leading-relaxed space-y-${isCompact ? '3' : '4'}`}>
      {blocks.map((block, bi) => {
