export const SERVICE_DETAIL_BLOCKS_SCHEMA_VERSION = 1;
...
function normalizeSteps(steps) {
  return list(steps)
    .map((item, index) => {
      ...
      return step.name || step.desc || image ? step : null;
    })
    .filter(Boolean);
}
...
function normalizeProofCards(cards) {
  return list(cards)
    .map((item, index) => {
      if (!isObject(item)) return null;
      const media = normalizeMedia(item.media || item.visual, `metric-media-${index + 1}`);
      const card = {
      ...
      return card.title || card.desc || media ? card : null;
    })
    .filter(Boolean);
}
...
function normalizeTestimonials(testimonials, legacyQuote = '') {
  const normalized = list(testimonials)
    .map((item, index) => {
      if (!isObject(item)) return null;
      const media = normalizeMedia(item.media || item.visual, `testimonial-media-${index + 1}`);
      ...
      return testimonial.quote || testimonial.author || testimonial.role || media ? testimonial : null;
    })
    .filter(Boolean);
  ...
  return list(items)
    .map((item, index) => {
      ...
      switch (type) {
        ...
        case 'quote': {
          const media = normalizeMedia(item.media || item.visual, `story-quote-media-${index + 1}`);
          const quote = text(item.quote, text(item.body, text(item.content)));
          if (!quote && !media) return null;
          return {
            ...base,
            quote,
            author: text(item.author, text(item.name)),
        ...
        case 'metric': {
          const media = normalizeMedia(item.media || item.visual, `story-metric-media-${index + 1}`);
...
export function createServiceDetailBlock(type, overrides = {}) {
  ...
  switch (safeType) {
    case 'intro':
      return { ...base, variant: overrides.variant || 'default', headline: '', summary: '', badges: [], ...overrides };
    case 'rich_text':
      return { ...base, body: '', ...overrides };
    case 'effects':
      return { ...base, cards: [{ title: '', desc: '', icon: '' }], ...overrides };
    case 'process':
      return { ...base, steps: [{ step: '01', name: '', desc: '', image: null }], ...overrides };
    case 'gallery':
    ...
    case 'youtube_embed':
      return { ...base, url: '', video_id: '', title: '', description: '', aspect_ratio: '16:9', ...overrides };
    case 'video':
      return { ...base, url: '', poster_url: '', title: '', aspect_ratio: '16:9', ...overrides };
    case 'sub_products':
    ...
    case 'related_magazine':
      return { ...base, magazine_slug: '', header: '', ...overrides };
    case 'landing_section':
      return { ...base, section_type: 'services', title: '', subtitle: '', content: {}, source_label: '', ...overrides };
    case 'cta':
      return { ...base, copy: '', button_label: '상담 문의하기', button_href: '/contact', ...overrides };
    default:
      return { ...base, body: '', ...overrides };
  }
}
...
export function normalizeServiceDetailBlock(block, index = 0) {
  ...
  switch (block.type) {
    ...
    case 'rich_text':
      return { block: { ...common, ...normalizeStoryFields(block), body: text(block.body, text(block.content)) }, errors: [] };
    case 'effects':
      return { block: { ...common, ...normalizeStoryFields(block), cards: normalizeCards(block.cards || block.items) }, errors: [] };
    case 'process':
      return { block: { ...common, ...normalizeStoryFields(block), steps: normalizeSteps(block.steps || block.items) }, errors: [] };
    case 'gallery':
      return {
        block: {
          ...common,
