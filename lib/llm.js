...
brief 의 content_angles, recommended_title, reader_problem 작성 시 위 페인포인트·후킹을 적극 반영할 것.` : '';
  ...
  // 주제 힌트(themeHint.target_topic_cluster)가 있으면 거기에 묶인 지식베이스를 함께 주입.
  // 예: place_visibility 주제는 플레이스 마케팅 KB + 거버넌스를 LLM 컨텍스트에 동봉.
  let knowledgeBlock = '';
  if (themeHint?.target_topic_cluster) {
    try {
      const { buildKnowledgeContext } = await import('@/lib/knowledge/loader');
      const kb = buildKnowledgeContext({ topicCluster: themeHint.target_topic_cluster });
      if (kb) knowledgeBlock = `\n\n[참조 지식베이스 — 표현·금지어·실무 분류 기준]\n${kb}`;
    } catch {
...
주 타겟 고객은 "고객 유입이 막혀 마케팅 방식을 바꿔야 하나 고민하는 작은 사업자"다.
대표 업종은 요식업과 병의원이지만, 핵심 문제는 업종 구분보다 "어떻게 고객을 끌어오고, 신뢰를 만들고, 상담/방문으로 전환할 것인가"다.
...
3) 콘텐츠 기획 브리프 (brief): 이 원문을 우리 타겟 대상 매거진/뉴스레터/블로그 글로 변환할 때 쓸 정보.
   ...
   - business_contexts: ["restaurant" | "clinic" | "local_service" | "general"] 중 1~3개.
   ...
   - reader_problem: 독자가 겪는 실무 문제. 예: "광고비는 쓰는데 신규 문의가 늘지 않는다"
...
JSON 형태:
{
  ...
  const VALID_CONTEXTS = ['restaurant', 'clinic', 'local_service', 'general'];
