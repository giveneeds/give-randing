...
import { VALID_PERSONAS, CLUSTERS_BY_PERSONA, normalizePersona } from '@/lib/contentTaxonomy';
...
- 운영자 페인포인트: ${(researchContext.pain_points || []).join(' / ')}
...
주 타겟 고객은 "마케팅을 직접 보고 판단해야 하는 작은 사업자/브랜드 운영자"다.
요식업·병의원·뷰티·교육·로컬 브랜드는 서로 다른 페르소나가 아니라 예시 맥락이다. 뉴스성/브랜드 사례 소재는 자영업자로 단정하지 말고, 브랜드와 콘텐츠를 운영하는 사람의 판단 문제로 넓게 본다.

콘텐츠 톤:
- 독자가 바로 자기 업체에 적용해볼 수 있게 쉽게 풀어쓴다.
- 업종 호칭은 원문이 그 업종을 명확히 가리킬 때만 쓴다.
- 너무 전문가 관점의 일반론보다 "지금 내가 운영하는 화면/콘텐츠/고객 동선에서 무엇을 봐야 하는가"가 보여야 한다.
...
3) 콘텐츠 기획 브리프 (brief): 이 원문을 우리 타겟 대상 매거진/뉴스레터/블로그 글로 변환할 때 쓸 정보.
   - target_persona: "general" | "unknown"
     · general = 작은 사업자/브랜드 운영자/마케팅을 직접 보는 사람. 브랜드·소비자·플랫폼 관찰 소재도 이 값으로 둔다.
     · unknown = 누구에게 필요한지 불명확함.
   - business_contexts: ["restaurant" | "clinic" | "local_service" | "brand" | "marketing" | "general"] 중 1~3개.
   - topic_cluster: 페르소나에 맞는 클러스터 한 개 또는 null.
     · general: place_visibility | review_trust | ad_efficiency | menu_offer | local_retention | service_page | local_acquisition | content_showcase | trend_plain | consumer_behavior
     · unknown 이면 null
...
- brief: 위 시스템 메시지 정의대로 채울 것. 업종 특화 소재가 아니면 자영업자/요식업/병의원으로 단정하지 말고 작은 사업자/브랜드 운영자 관점에서 작성.
...
JSON 형태:
{
  ...
  if (parsed.brief && typeof parsed.brief === 'object') {
    ...
    const persona = VALID_PERSONA.includes(b.target_persona)
      ? b.target_persona
      : normalizePersona(b.target_persona);
    ...
    const rawFit = typeof b.fit_score === 'number' ? Math.max(0, Math.min(1, b.fit_score)) : null;
    // 구체성 가중: 본문/제목/브리프에 *전술 키워드*(수치·도구명·명령·Before·After·실 사용자 후기 신호)
    // 가 많고 *추상 키워드*(미래·트렌드·시대·역량·전망·패러다임·혁신·중요하다) 가 적을수록 fit ↑.
    // thought leadership 류 추상 기사가 후보 풀에서 밀려나도록.
    const tacticalSignal = computeTacticalSignal({ title, text, brief: b });
    const fit = (rawFit !== null) ? Math.max(0, Math.min(1, rawFit * tacticalSignal.multiplier)) : null;
    ...
    brief = {
      ...
      business_contexts: contexts.length > 0 ? contexts : (persona === 'general' ? ['brand', 'marketing'] : ['general']),
...
// 구체성 가중치 계산기 — 추상 키워드 ↓ / 전술 키워드 ↑.
// 산출: { multiplier: 0.6~1.4, tactical_hits: number, abstract_hits: number, samples: {tactical:[], abstract:[]} }
// 곱셈 모델 — 원본 fit 의 의미를 유지하면서 *후보 풀의 추상 비중을 깎는 방향* 으로 동작.
const TACTICAL_KEYWORDS = [
  // 수치 시그널
  /\d+\s*(%|배|만\s?원|만\s?명|건|시간|초|분|일|주|개월|개|점)/g,
  // 도구·플랫폼 고유명 (대소문자 무시)
  /(GA4|GPT|Claude|Gemini|Cursor|Midjourney|Canva|Apify|Tavily|Perplexity|Slack|메타\s?광고|네이버\s?광고|구글\s?애즈|쿠팡)/gi,
  // 실 사용자 워크플로우/명령 시그널
  /(프롬프트|명령|이렇게\s?했|이런\s?식으로|단계별|워크플로우|이걸\s?넣었|이\s?설정|이\s?옵션)/g,
  // 결과/측정 시그널
  /(전환율|이탈률|ROI|ROAS|CTR|CPC|CPA|클릭률|체류시간|예약\s?전환|매출\s?증가|반응이?\s?(터졌|좋|줄))/g,
  // Before/After 시그널
  /(이전엔|예전엔|원래|바꿨더니|바꾸니|돌리니|돌려보니|넣었더니|를\s?넣어서|로\s?바꾸고|로\s?고치고)/g,
  // 실제 후기 시그널
  /(해\s?봤|돌려\s?봤|써\s?봤|적용해\s?보니|적용하니|운영해\s?보니)/g,
];
const ABSTRACT_KEYWORDS = [
  /(미래\s?의|새로운\s?시대|새로운\s?패러다임|혁신적|중요해지고\s?있|필요해지고\s?있|달라지고\s?있)/g,
  /(역량|전망|트렌드|시대|방향성|핵심\s?가치|본질|차별점|패러다임)/g,
  /(고려해야|점검해야|중요합니다|필요합니다|시작해야|준비해야|구분해야)/g,
  /(전략적\s?사고|창의성을?\s?(증진|키우|강화)|감성\s?지능|의사결정\s?역량)/g,
  /(thought\s?leadership|paradigm|the\s?future\s?of|key\s?capabilit)/gi,
];
...
function countMatches(text, patterns) {
  ...
  return { count: total, samples: samples.slice(0, 6) };
}

function computeTacticalSignal({ title, text, brief }) {
  // 분석 대상: 제목 + 본문 앞 2000자 + brief 의 핵심 텍스트 (LLM 이 만든 요약·앵글·문제정의).
  const briefBlob = [
    ...
    brief?.why_now,
    brief?.content_angle,
    ...(Array.isArray(brief?.content_angles) ? brief.content_angles : []),
    brief?.practical_takeaway,
    ...(Array.isArray(brief?.execution_steps) ? brief.execution_steps : []),
    brief?.approval_reason,
  ].filter((x) => typeof x === 'string').join(' ');
  const blob = `${title || ''} ${(text || '').slice(0, 2000)} ${briefBlob}`;

  const tactical = countMatches(blob, TACTICAL_KEYWORDS);
  const abstract = countMatches(blob, ABSTRACT_KEYWORDS);

  // 점수 산식 — 전술 hit 당 +0.05, 추상 hit 당 -0.06, baseline 1.0, clamp [0.6, 1.4].
  // 추상 페널티가 살짝 큰 이유: 추상 키워드 1개 들어가면 글 톤이 *통째로 일반론* 으로 흐를 확률이 높음.
  let multiplier = 1.0 + tactical.count * 0.05 - abstract.count * 0.06;
  multiplier = Math.max(0.6, Math.min(1.4, multiplier));

  return {
    multiplier: Math.round(multiplier * 100) / 100,
    tactical_hits: tactical.count,
    abstract_hits: abstract.count,
    samples: {
      tactical: tactical.samples,
      abstract: abstract.samples,
    },
  };
}
