...
function buildWriterPrompt({ contentPlan, evidenceSnapshot, referenceContext, sourceArticleText }) {
  ...
  if (isIssueExplainer) {
    ...
    return [
      ...
      '- 전문가 발언/팁 이슈도 7개 안에서 완결하세요. 장면, 근거, 전환, 의미를 압축해 7개에 담습니다.',
      ...
      '- 대상을 “작은 사업자”, “소상공인”, “타깃 고객” 같은 직역 분류명으로 부르지 마세요. 한국 사람이 실제 쓰는 호칭(사장님 등)이나 행동을 묘사한 호칭(“혼자 광고까지 챙기는 사장님”)으로 부릅니다. 단 마케터, 자영업자처럼 이미 자연스러운 말은 그대로 둡니다.',
      ...
      '- 전문가 발언/팁 이슈는 같은 issue_explainer 안에서 이렇게 풀 수 있습니다: 강한 첫 말, 진짜 질문, 대부분의 오해, 핵심 논리, 머릿속에서 벌어지는 일, 일상 장면, 중간 요약, 오늘 할 행동.',
      '- 이 경우 독자는 직업명이 아니라 심리 상태로 잡아도 됩니다. 예: “머릿속엔 많은데 말로 못 꺼내는 사람”, “회의 끝나고 뒤늦게 할 말 생각나는 사람”.',
      '',
      '말투:',
      '- 뉴스 기사체, 보고서체, 존댓말 설명체로 쓰지 마세요.',
      '- 짧은 행갈이와 단정적인 구어체를 씁니다. 예: “올라왔음.”, “줄 섰음.”, “이유는 따로 있음.”',
      '- “~입니다/합니다/볼 수 있습니다”를 기본 문체로 쓰지 마세요. 필요할 때만 아주 적게 씁니다.',
      '- 이모지는 절대 쓰지 않습니다. 0개. 감정과 강조는 단어와 문장 구조로만 표현합니다.',
      '- 문장 끝은 “했음/나옴/보임/거임/뜻임/자리” 같은 구어체를 섞되, 전부 같은 어미로 반복하지 마세요.',
      '- “했음”만 반복하면 어색합니다. “올라왔다”, “붙어 있었다”, “줄 선 거임”, “뜻임”, “그 얘기다”처럼 자연스럽게 섞으세요.',
      '- 문장을 명사로 끝내지 마세요. “보도.”, “주장.”, “분석.”, “해석.”, “요지.”, “흐름.”, “신호.” 같은 명사 종결은 실패입니다. 눈으로 빠르게 읽히지 않습니다.',
      '- 명사 종결은 서술형으로 풉니다. “보도됐음”, “~라고 주장했음”, “~로 분석함”, “~라는 게 핵심임”, “~라는 신호임”처럼 어간에 ~음/함/임/됨을 붙여 끝냅니다.',
      '- 인용, 근거도 명사로 끊지 말고 서술형으로 씁니다. “행사 발표로 알려진 바임.”, “업체 측 설명에 기반한 주장으로 보면 됨.”처럼 씁니다.',
      ...
      '- 줄 길이는 인위적으로 자르지 마세요. 짧은 줄(8~15자)과 긴 줄(30~60자)이 자연스럽게 섞이게 씁니다. 내용에 맞게 호흡이 결정되는 겁니다.',
      '- 날짜나 수치를 비교할 때는 한 줄씩 줄바꿈으로 나란히 씁니다. 예: "2024년 12월: 매출 5조\n2025년 5월: 매출 27조" 처럼요. 같은 줄에 쉼표로 이어 쓰지 마세요.',
      '- 전환이나 반전을 표시할 때 "여기서 한 번 뒤집힘." 또는 "근데 여기서 반전."처럼 단독 문장으로 쓸 수 있습니다.',
      '- "그러나", "따라서", "하지만"은 쓰지 마세요. 전환이 필요하면 "근데"를 쓰세요. 예: "근데 여기서 또 있음."',
      '- 영문 원문이 있으면 번역 전에 원문 일부를 그대로 써도 됩니다. 예: "Sequoia는 이걸 \'generational opportunity\'라고 불렀음." 이후 한국 관점 해석이 이어져야 합니다.',
      '- 번호식 설명, 화살표 설명, 압축 요약 문장을 남발하지 마세요. “1) 질문→의도 파악 2) 후보 군집→요약” 같은 틀은 가능하면 장면 묘사로 풀어 쓰세요.',
      '- 번호 나열 1) 2) 3)은 한 글에서 최대 한 번, 정보가 실제로 병렬일 때만 씁니다. 여러 post를 나열로 채우지 마세요. 나열이 두 번 필요해 보이면 한쪽은 서술 흐름으로 풉니다.',
      ...
      '',
      '자연스러운 흐름 (슬롯 채우기 금지):',
      '- 먼저 사람이 자연스럽게 쓴 한 편의 글을 완성한다고 생각하세요. 그 글을 소재에 맞는 포스트 수로 나누는 것이지, 슬롯을 따로 채워 이어붙이는 게 아닙니다.',
      '- 글의 구조나 진행을 독자에게 설명하는 메타 문장을 쓰지 마세요. “장면 하나 고정하자.”, “정리부터 하자.”, “받침은 이렇게 나온다.”, “여기서 포인트가 나온다.”처럼 내용 없이 구조만 가리키는 문장은 실패입니다. 바로 그 장면이나 사실을 쓰세요.',
      '- 구체 대상이 없는 추상 비유를 쓰지 마세요. “어디에 먼저 꽂았는가”, “축이 옮겨감”, “판이 바뀜”처럼 누가 무엇을 했는지 안 보이는 문장은 실패입니다. 주어와 대상을 구체 명사로 박으세요.',
      '- “그래서 뭐가 문제냐?” 같은 전환 질문은 쓸 수 있지만, 바로 다음 줄에서 구체적인 답을 줘야 합니다. “그래서 뭐가 갈랐냐? 어디에 먼저 꽂았는가였음.”처럼 추상 비유로 답하면 실패입니다.',
      '- 각 문장은 앞 문장의 구체 대상을 이어받습니다. “이게”, “그게”, “이 부분”처럼 지시어만 던지고 무엇을 가리키는지 비우지 마세요.',
      '- 조사와 서술어가 끊기지 않게 이어 쓰세요. 번역체로 명사구만 나열하지 말고, 누가 무엇을 어떻게 했는지 문장으로 풉니다.',
      '- 읽었을 때 한 사람이 쭉 이어서 쓴 글로 읽혀야 합니다. 카드 경계에서 화제가 튀거나, 앞 내용과 따로 노는 문장이 있으면 실패입니다.',
      '',
      'Narrative beats:',
      ...
      '',
      'Beat 운영 규칙:',
      '- beat 개수와 post 개수를 1:1로 맞추지 않아도 됩니다. 최종 post 수는 소재와 무관하게 정확히 7개로 고정합니다.',
      ...
      '- 7개 구성 예: 1 멈추는 장면/독자 호출, 2 원천 디테일 + 수치, 3 구조적 반전 또는 모순 장면, 4 왜 한 번짜리가 아닌가, 5 숫자/반응/시장 근거, 6 한국 관점 또는 독자 결론, 7 마무리 + 출처.',
