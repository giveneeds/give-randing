...
function buildPolishPrompt({ draft, contentPlan, critiqueReport }) {
  ...
  return [
    ...
    '',
    '자연스러운 흐름:',
    '- 글의 구조를 설명하는 메타 문장은 제거합니다. “장면 하나 고정하자.”, “받침은 이렇게 나온다.”, “정리부터 하자.”, “한 줄로 닫음.”, “마무리합니다.”, “이렇게 정리할 수 있음.”, “근데 어제 장면 하나.”, “핵심 문장 하나.”, “여기서 우리 일로 당겨보자.”처럼 내용 없이 구조만 가리키는 문장은 바로 그 사실 문장으로 바꾸거나 삭제합니다.',
    '- 유보/면책 문장(“더 확인이 필요함”, “공개된 범위만 확인됨”, “각자 체크 필요”, “제한이 있다고 언급됨”)이 두 번 이상 나오면 가장 중요한 한 곳만 남기고 제거합니다. 매 post마다 면책이 붙으면 글쓴이도 모른다는 인상만 남습니다.',
    '- 기사 자체의 메타데이터 서술(언제 공개됐는지, 인터뷰 형식인지, 1인칭 에세이인지, “장면감이 있었음” 같은 형식 평가)은 삭제하거나 인물의 실제 발언/행동으로 바꿉니다.',
    ...
    '- 링크 앞에 "원문 확인용 1차 링크", "출처:", "참고 링크", "원문:" 같은 레이블을 붙이지 않습니다. URL만 단독으로 씁니다.',
    '',
    'AI 습관 제거 (번역투·AI 말투 교정):',
    '- 연결 어미 뒤 쉼표를 삭제합니다. "-고," "-며," "-지만," "-면서," "-는데," 처럼 연결 어미 바로 뒤 쉼표는 AI 글의 가장 강한 신호입니다. 전부 제거합니다.',
    '- 이중 피동을 단순화합니다. "되어지다" → "되다", "판단되어진다" → "판단된다", "알려져 있다" → "알려졌다".',
    '- "~에 의해" 수동태는 능동형으로 전환합니다. "FTC에 의해 제재됐음" → "FTC가 제재했음".',
    '- "그/그녀/그것/그들"처럼 영어 대명사를 번역한 표현은 삭제하거나 문맥 명사로 바꿉니다. 한국어는 주어 생략이 자연스럽습니다.',
    '- 경동사를 단순화합니다. "경쟁력을 가지고 있다" → "경쟁력이 있다". "영향을 미치고 있다" → "영향을 미친다".',
    '- "~를 통해"는 더 직접적인 표현으로 바꿉니다. "광고를 통해 매출을 올림" → "광고로 매출을 올림".',
    '- 아래 시그니처 구문은 삭제하거나 구체 문장으로 교체합니다: "결론적으로", "요약하면", "혁신적인", "시사하는 바가 크다", "~의 지평을 열다", "~시대가 도래했다", "주목할 만하다", "중요한 시사점을 제공한다".',
    '- 과도한 유보 표현을 해소합니다. "~할 수 있을 것으로 보인다" → "~로 보임". "~라고 할 수 있겠다" → "~임". 사실 기반 문장은 단정적으로 씁니다.',
    '- "또한"으로 시작하는 문장은 삭제하거나 "여기에", "거기에", "더해"로 교체합니다. "또한"이 3개 이상이면 70% 삭제합니다.',
    '- "~라는 것이다", "~는 것임"으로 끝나는 문장은 직접 서술로 바꿉니다. "중요하다는 것이다" → "중요함". "핵심이라는 것임" → "핵심임".',
    '- "-적/-성/-화" 접미사 체인은 풀어씁니다. "혁신적 변화" → "뿌리부터 바뀐다". "효율적 운영" → "빠르게 돌아가는 운영". "자동화 솔루션" → "자동으로 돌아가는 도구".',
    '',
    '수정 금지:',
    '- 사실관계 변경 금지',
    '- 수치 변경 금지',
