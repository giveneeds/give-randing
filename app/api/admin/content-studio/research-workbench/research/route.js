...
function buildPrompt(item, contentPlan) {
  ...
  if (isIssueExplainer) {
    return [
      ...
      '- 1차 출처(공식 발표, 보도자료 원문, 당사자 발언, 원자료, 기관 블로그 원문)를 기사 요약이나 2차 보도보다 우선하세요.',
      '- 각 finding에는 그 사실이 나온 1차 출처 URL을 source_url에 정확히 넣으세요. 1차 출처가 없으면 가장 신뢰할 수 있는 출처 URL을 넣고, 추측해서 만들지 마세요.',
      ...
      '- 전문가 발언/팁 이슈라면 원문 인용, 그 말이 나온 맥락, 핵심 논리, 일상 적용 사례, 흔한 오해를 우선 찾으세요.',
      ...
      '      "source_domain": "<도메인만>",',
      '      "source_url": "<이 사실의 1차 출처 전체 URL. 추측 금지>",',
      '      "evidence_type": "source_origin | case_detail | mechanism | official_position | public_reaction | risk_check | expert_quote | platform_policy | original_quote | concept_backbone | practical_example",',
  ...
  return [
    ...
    '- 1차 출처(공식 문서 원문, 보도자료, 당사자 발언)를 2차 요약보다 우선하고, 각 finding의 source_url에 그 1차 출처 URL을 넣으세요.',
