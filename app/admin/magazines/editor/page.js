...
export default function MagazineEditor() {
  ...
  const [magazine, setMagazine] = useState({
    ...
    is_premium: false, is_published: true, author: 'GIVENEEDS'
  });
      ...
      <div className="flex-1 flex overflow-hidden">
        <aside className="w-full max-w-[450px] border-r border-zinc-100 bg-zinc-50/50 flex flex-col overflow-y-auto p-10 space-y-12 shrink-0 custom-scrollbar">
           <div className="space-y-4">
              ...
              <input className="w-full p-4 bg-white border border-zinc-200 rounded-md font-bold text-base outline-none shadow-sm" placeholder="기사 제목을 입력하세요" value={magazine.title} onChange={e => setMagazine({ ...magazine, title: e.target.value })} />
              <div className="grid grid-cols-2 gap-4">
                 <select className="w-full p-4 bg-white border border-zinc-200 rounded-md font-bold text-xs uppercase tracking-widest outline-none shadow-sm" value={magazine.category} onChange={e => setMagazine({ ...magazine, category: e.target.value })}>
                   <option value="INSIGHT">Insight</option><option value="STRATEGY">Strategy</option><option value="ANALYSIS">Analysis</option>
                 </select>
                 <input className="w-full p-4 bg-white border border-zinc-200 rounded-md font-mono text-[11px] outline-none shadow-sm" placeholder="my-article-slug" value={magazine.slug} onChange={e => setMagazine({ ...magazine, slug: e.target.value })} />
              </div>
              <input className="w-full p-4 bg-white border border-zinc-200 rounded-md font-mono text-[11px] outline-none shadow-sm" placeholder="Thumbnail Image URL" value={magazine.thumbnail_url} onChange={e => setMagazine({ ...magazine, thumbnail_url: e.target.value })} />
           </div>

           <div className="flex-1 space-y-4 pt-10 border-t border-zinc-200">
              <label className="text-[10px] font-bold text-zinc-900 uppercase ml-1">Content Editor</label>
              <textarea className="w-full min-h-[400px] bg-white border border-zinc-200 rounded-md p-8 text-sm leading-relaxed outline-none font-mono shadow-sm" placeholder="본문을 마크다운이나 HTML로 작성하세요..." value={magazine.content_html} onChange={e => setMagazine({ ...magazine, content_html: e.target.value })} />
           </div>

           <div className="pt-10 border-t border-zinc-200 space-y-3">
    ...
    <div className="bg-white min-h-full pb-32 animate-in fade-in duration-700">
       <div className="pt-24 px-8 max-w-screen-md mx-auto mb-16">
          <div className="flex items-center gap-4 mb-8">
             <span className="text-[10px] font-bold tracking-[0.3em] text-zinc-400 uppercase">{post.category} — ISSUE 2025</span>
             {post.is_premium && <span className="bg-zinc-900 text-white text-[9px] font-black px-2 py-0.5 rounded-sm uppercase">PREMIUM</span>}
          </div>
          <h1 className="text-4xl md:text-5xl font-black leading-[1.05] tracking-tighter text-zinc-900 mb-8 break-keep">{post.title}</h1>
          <div className="h-px bg-zinc-100 w-full" />
       </div>
       {post.thumbnail_url && <div className="px-8 max-w-screen-md mx-auto mb-16"><div className="aspect-[16/9] bg-zinc-50 border border-zinc-100 rounded-sm overflow-hidden"><img src={post.thumbnail_url} className="w-full h-full object-cover" /></div></div>}
