...
import {
  ...
  Sparkles, Clock, Hash, FileText, GitBranch, ShieldCheck, ListChecks, Play,
} from 'lucide-react';
      ...
      {data.sessions.length === 0 ? (
        ...
        <div className="space-y-6">
          {data.sessions.map((s, idx) => (
            <SessionCard key={s.id} session={s} index={idx + 1} authHeaders={authHeaders} />
          ))}
        </div>
      )}
...
function SessionCard({ session, index, authHeaders }) {
  ...
  const awaitingSelection = !session.selected_item_id
    && (session.status === 'phase1_reported' || session.status === 'awaiting_decision');
  const candidateList = awaitingSelection && session.candidates_summary
    ? Object.values(session.candidates_summary).sort((a, b) => (a.candidate_index || 999) - (b.candidate_index || 999))
    : [];

  async function runFinish(itemId) {
    setFinishing(itemId); setFinishErr('');
    try {
      const res = await fetch(`/api/admin/content-studio/sessions/${session.id}/finish`, {
        method: 'POST',
        headers: await authHeaders(),
        body: JSON.stringify({ selectedItemId: itemId }),
      });
      const d = await res.json();
      if (!res.ok) throw new Error(d.error || '발행 실패');
      router.push(`/admin/content-studio/sessions/${session.id}`);
    } catch (e) {
      setFinishErr(e.message);
      setFinishing('');
    }
  }
        ...
        {awaitingSelection ? (
          <div>
            <p className="text-sm text-[var(--admin-text-muted)] mb-3">
              아직 후보를 채택하지 않았습니다. 발행할 후보를 골라 파이프라인을 실행하세요.
            </p>
            {finishErr && <p className="mb-2 text-xs font-bold text-red-600">{finishErr}</p>}
            <div className="grid grid-cols-1 gap-2">
              {candidateList.map((c) => (
                <div key={c.id} className="flex items-start justify-between gap-3 rounded-lg border border-[var(--admin-border)] bg-white p-3">
                  <div className="min-w-0">
                    <div className="text-xs text-[var(--admin-text-muted)]">
                      {c.label || `[후보 ${c.candidate_index || '?'}]`}
                      {typeof c.fit_score === 'number' && <> · 적합도 {Math.round(c.fit_score * 100)}%</>}
                      {(c.selected_content_pillar_label || c.selected_content_pillar) && <> · {c.selected_content_pillar_label || c.selected_content_pillar}</>}
                    </div>
                    <div className="font-bold text-sm mt-0.5 line-clamp-2">{c.recommended_title || c.title || '(제목 없음)'}</div>
                  </div>
                  <button
                    type="button"
                    disabled={!!finishing}
                    onClick={() => runFinish(c.id)}
                    className="inline-flex items-center gap-1.5 shrink-0 px-3 py-2 rounded-md bg-violet-600 text-white text-xs font-bold hover:bg-violet-700 disabled:opacity-50"
                  >
                    {finishing === c.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
                    이 후보로 발행
                  </button>
                </div>
              ))}
            ...
            {finishing && (
              <p className="mt-3 text-xs text-[var(--admin-text-muted)]">
                글 생성에는 수 분이 걸릴 수 있습니다. 완료되면 진행 과정 화면으로 이동합니다.
              </p>
            )}
          </div>
        ) : drafts.length === 0 ? (
