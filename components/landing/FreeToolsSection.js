...
export default function FreeToolsSection() {
  return (
    <section id="tools" className="py-20 sm:py-32 bg-white">
      <div className="px-4 sm:px-6 md:px-12 max-w-screen-xl mx-auto">
        <div className="mb-12 sm:mb-20 text-center md:text-left">
          ...
          <h2 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tighter text-zinc-900 leading-[1.1]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-zinc-100 border border-zinc-100">
          {tools.map((tool, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`group p-6 sm:p-12 ${tool.color} border border-zinc-100 flex flex-col justify-between transition-all hover:bg-zinc-900 hover:text-white`}
            >
              <div>
                <tool.icon className="mb-6 sm:mb-10 w-9 h-9 sm:w-10 sm:h-10 text-zinc-300 group-hover:text-zinc-600 transition-colors" />
              ...
              <button className="mt-8 sm:mt-12 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-900 group-hover:text-white transition-all">
