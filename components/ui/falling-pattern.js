...
export function FallingPattern({
  ...
  density = 0.5,
  animationDuration = 30, // Default slightly faster for visibility
  blurIntensity = '1px',
}) {
  ...
  const generateBackgroundImage = () => {
    const patterns = [
      // Row 1
      `radial-gradient(4px 100px at 0px 220px, ${color}, transparent)`,
      `radial-gradient(4px 100px at 300px 220px, ${color}, transparent)`,
      `radial-gradient(1.5px 1.5px at 150px 110px, ${color} 100%, transparent 150%)`,
      // Row 2
  ...
  return (
    <div className={cn('relative w-full h-full p-1 overflow-hidden', className)}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
