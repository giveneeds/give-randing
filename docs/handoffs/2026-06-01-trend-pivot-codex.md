...
## 11. UPDATE 2026-06-01 (현재 시점) — Claude 가 [0] 박스까지 완료

위 1~10번 섹션은 트랙 *시작 직전* 의 큰 맥락. 이 섹션이 **코덱스가 실제로 이어받는 시작점**.

### 현재 commit 상태 (둘 다 로컬만, push X)

```
a2aee32  feat(content-studio): 트렌드 피벗 트랙 시작 — 안전망·워크벤치 첫 페이지·도식
fd8a448  chore(infra): turbopack root 명시 — 상위 폴더 bun.lock 영향 차단
2dfd8f7  feat(content-agent): D5 ...  ◀ git tag v-pre-trend-pivot (직전 origin/main)
```

→ origin/main 보다 2 commit 앞섬. **push 절대 금지** — 정욱님 명시 승인 시점까지 X.

### 안전망 — 전부 완료
- [x] git tag `v-pre-trend-pivot` 로컬 (commit 2dfd8f7)
- [x] `docs/experiments/` 폴더 + README + `.gitignore` 등록 (정욱님 본인용, 로컬 전용)
- [x] `.env.local` 에 `USE_TREND_PIPELINE=false` 스텁
- [x] `app/admin/content-studio/research-workbench/` 별도 모듈 — 기존 흐름 한 줄도 안 건드림

### 워크벤치 페이지 — [0] 박스까지 완료, [1] 부터 시작
경로: `app/admin/content-studio/research-workbench/page.js` (이미 빌드 통과)
어드민 → 콘텐츠 스튜디오 → **워크벤치** 탭 (Sparkles 아이콘) 클릭.

페이지 안 단계 진행도:
```
✓ [0] 내부 문서 + 관심 주제 시각화
☐ [1] LLM 에이전트 추론 — 다음 작업    ← 코덱스가 시작할 자리
☐ [2] 검색 프롬프트 자동 작성
☐ [3] 결과 + 검증 게이트
☐ [4] 기존 finishPlanningSession 으로 핸드오프
```

### 정욱님의 [0] 박스 검수 — 미진행
페이지가 만들어졌으나 정욱님이 *디자인·정보 양·문구 검수* 를 아직 못 함 (로컬 dev 서버 재시작 직후 코덱스로 넘김).
→ **코덱스 첫 작업 전에 정욱님에게 [0] 박스 본 소감·수정 요청 받을 것.**

내부 문서 4종 (페르소나·기둥·톤가이드·realbody) 펼침 시 *실제 파일 내용* 은 아직 안 뜸 (다음 단계에서 API 연결).

### [1] 단계 — 코덱스가 시작할 자리

목표: 에이전트가 *무엇을 찾을지* 추론하는 박스.

설계 (정욱님 명세):
- 입력: 트렌드 가설 (수동 textarea, 비워도 OK) + 관심 주제 체크박스 (6개 중 N개)
- 처리: 내부 문서 4종 + 관심 주제 + 트렌드 가설 → LLM (**Perplexity Agent API 우선**)
- 출력: 필요한 자료 목록 (사용자가 항목별 편집·추가·삭제 가능)
- 검증 게이트: 통과 안 하면 [2] 실행 X (토큰 절약)

작업 순서 (각 작은 PR/commit 단위):
1. UI mock — 트렌드 textarea + 체크박스 + "추론 시키기" 버튼 + 더미 자료 목록 출력
2. 실제 LLM 호출 — 새 API 라우트 `/api/admin/research-workbench/infer` 생성
3. 결과 표시 + 항목별 편집 UI
4. 검증 게이트 ("[2] 단계로 가기" 버튼 활성/비활성)

### Perplexity Agent API 정보 (정욱님이 사용 의도 명시)
- 정욱님이 **3종 Perplexity API 중 Agent API** 사용 의도 (다양한 모델 결합형, ChatGPT·Claude·Code 등 오퍼레이션 가능, 리서치 탁월).
- 기존 코드는 `lib/research/perplexityProvider.js` 가 **Sonar API** 사용 중.
- Agent API 키 추가·엔드포인트·인증 방식은 **코덱스가 정욱님에게 확인 후 진행**. `.env.local` 에 별도 환경변수로 (예: `PERPLEXITY_AGENT_API_KEY`).

### 로컬 환경 상태
- dev 서버: `npm run dev` (포트 3000). 방금 재시작 트리거. 코덱스가 받았을 때 응답 안 하면 다시 띄울 것.
- `.env.local` 변경: `USE_TREND_PIPELINE=false` 추가. 또 *실수로 PERPLEXITY_API_KEY 라인이 줄바꿈 없이 USE_TREND_PIPELINE 와 합쳐졌던 사고* 가 있어서 `sed` 로 복구. 백업: `.env.local.backup-*` (gitignore 됨).

### 코덱스 첫 응답에서 정욱님에게 할 일
1. "현재 시점 핸드오프 v1 + 11번 update 읽었음" 짧게 확인
2. 정욱님이 *워크벤치 [0] 박스 본 소감* 듣기 — 수정 요청 있으면 그것 먼저
3. [1] 단계 작업 들어가기 전 — Perplexity Agent API 키·엔드포인트·인증 방식 정욱님에게 확인
4. UI mock 부터 (LLM 0개) — 빌드 통과 + 정욱님 화면 확인 → 실제 API 연결

### 코덱스가 절대 잊지 말 것
- `git push origin main` 금지 (정욱님 명시 "push 해줘" 또는 "배포해줘" 받기 전까지)
- 코드 작성 시 항상 *프론트 골격 → 백 연결* 순서
- 기존 cron/`runCollection`/`finishPlanningSession` **한 줄도 안 지움**
- 일지·메모는 `docs/experiments/*.md` (로컬 전용, gitignore)
- 메모리 (`~/.claude/projects/.../memory/MEMORY.md`) 의 *작업 안전 규칙* 메모리 참조
